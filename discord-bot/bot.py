...
ADMIN_IDS: set[int] = {TRUSTED_USER_ID}   # IDs com acesso ao painel admin do site
...
def _verify_discord_token(token: str) -> str | None:
    """Verifica um token OAuth2 do Discord e retorna o user ID se válido."""
...
async def _handle_trusted_command(message: discord.Message) -> bool:
    ...
    # ── Tempo: libera acesso ao site manualmente ──
    # Uso: tempo ID [nick]  ou  tempo @menção [nick]
    if texto.startswith("tempo "):
        ...
        if len(partes) >= 2:
            ...
            if raw.isdigit():
                target_id = raw
                nick_arg = " ".join(partes[2:]) if len(partes) > 2 else f"ID {target_id}"
                # Libera imediatamente: registered_at = agora − 8 dias
                registered_at = (
                    discord.utils.utcnow() - datetime.timedelta(days=8)
                ).isoformat()
                form_registrations[target_id] = {
                    "nick": nick_arg,
                    "registered_at": registered_at,
                    "manual_grant": True,
                }
                _save_registrations()
                await reply(f"✅ Acesso liberado para `{target_id}` ({nick_arg})!")
            else:
                await reply("❌ Uso: `tempo <ID_DO_DISCORD> [nick]`")
        else:
            await reply("❌ Uso: `tempo <ID_DO_DISCORD> [nick]`")
        return True
    ...
    if texto in ["ajuda", "help", "comandos"]:
        await reply(
            ...
            "`tempo [ID] [nick]` — libera acesso ao site manualmente\n"
        )
        return True
...
async def on_member_remove(member: discord.Member) -> None:
    """Detecta saída de membros em tempo real e notifica no Discord."""
    ...
    if channel:
        embed = discord.Embed(
            description=(
                ...
                f"O acesso ao site foi bloqueado automaticamente."
            ),
            color=0xe74c3c,
    ...
    print(f"[SAÍDA] {member.display_name} ({member.id}) saiu do servidor.")
...
# ──────────────────────────────────────────────
# Endpoints — painel admin do site
# ──────────────────────────────────────────────
async def handle_admin_grant(request: web.Request) -> web.Response:
    """POST /admin/grant — libera ou ajusta acesso de um membro."""
    try:
        data = await request.json()
    except Exception:
        return web.json_response({"error": "JSON inválido"}, status=400, headers=CORS_HEADERS)

    discord_token = data.get("discord_token", "")
    caller_id = await asyncio.to_thread(_verify_discord_token, discord_token)
    if not caller_id or int(caller_id) not in ADMIN_IDS:
        return web.json_response({"error": "Não autorizado"}, status=403, headers=CORS_HEADERS)

    target_id    = str(data.get("discord_id", "")).strip()
    nick         = data.get("nick") or f"ID {target_id}"
    registered_at = data.get("registered_at")

    if not target_id or not registered_at:
        return web.json_response(
            {"error": "discord_id e registered_at são obrigatórios"}, status=400, headers=CORS_HEADERS
        )
    ...
    _save_registrations()
    print(f"[ADMIN] Acesso ajustado para {target_id} ({nick}) por admin {caller_id}")
    return web.json_response({"success": True, "discord_id": target_id}, headers=CORS_HEADERS)


async def handle_admin_list(request: web.Request) -> web.Response:
    """GET /admin/list?token=... — lista todos os registros."""
    discord_token = request.query.get("token", "")
    caller_id = await asyncio.to_thread(_verify_discord_token, discord_token)
    if not caller_id or int(caller_id) not in ADMIN_IDS:
        return web.json_response({"error": "Não autorizado"}, status=403, headers=CORS_HEADERS)
    ...
    guild = bot.get_guild(GUILD_ID)
    result = []
    for did, reg in form_registrations.items():
        is_member = False
        if guild:
            try:
                is_member = guild.get_member(int(did)) is not None
            except Exception:
                pass
        result.append({
            "discord_id": did,
            "nick": reg.get("nick", "—"),
            "registered_at": reg.get("registered_at", ""),
            "manual_grant": reg.get("manual_grant", False),
            "member": is_member,
        })
...
async def handle_admin_options(request: web.Request) -> web.Response:
    return web.Response(headers=CORS_HEADERS)
...
async def _main() -> None:
    ...
    app.router.add_post("/admin/grant", handle_admin_grant)
    app.router.add_get("/admin/list", handle_admin_list)
    app.router.add_route("OPTIONS", "/admin/grant", handle_admin_options)
    app.router.add_route("OPTIONS", "/admin/list", handle_admin_options)
    ...
    print(f"[OK]   POST /webhook/forms   — inscrições do Google Forms")
    print(f"[OK]   GET  /verify/{{id}}     — verifica acesso (site)")
    print(f"[OK]   POST /admin/grant     — painel admin: ajusta acesso")
    print(f"[OK]   GET  /admin/list      — painel admin: lista membros")
